...
- **Install / distribution**: ships as **source only** (private repo — bundles Clawd/Anthropic assets, no public installers). `./install.sh` is the one-command path (checks prereqs, installs Rust if missing, builds, copies `.app` to /Applications, de-quarantines, opens). `bundle.targets` = `["app","appimage"]` (no `.dmg`). CI is `.github/workflows/ci.yml` — `cargo test` gate only. Linux: macOS-only deps (`objc`, `tauri-nspanel`) are target-gated; token falls back to `~/.claude/.credentials.json`; PiP degrades to always-on-top + all-workspaces.
